...
if (!extension_loaded('mysqli')) {
    ...
    $prefix = (PHP_SHLIB_SUFFIX == 'so')
        ? ''
        : 'php_';
        require_once '../config/mysqli.so.php';
}
...
$MYSQL2PHP_TYPE_INT = array(
    MYSQL_TYPE_TINY, MYSQL_TYPE_SHORT, MYSQL_TYPE_LONG, MYSQL_TYPE_LONGLONG,
    MYSQL_TYPE_INT24, MYSQL_TYPE_YEAR
);
$MYSQL2PHP_TYPE_FLOAT = array(
    MYSQL_TYPE_DECIMAL, MYSQL_TYPE_FLOAT, MYSQL_TYPE_DOUBLE
);
$MYSQL2PHP_TYPE_STRING = array(
    MYSQL_TYPE_TINY_BLOB, MYSQL_TYPE_MEDIUM_BLOB, MYSQL_TYPE_LONG_BLOB,
    MYSQL_TYPE_BLOB, MYSQL_TYPE_VAR_STRING, MYSQL_TYPE_STRING,
    MYSQL_TYPE_GEOMETRY, MYSQL_TYPE_SET, MYSQL_TYPE_ENUM, MYSQL_TYPE_DATE,
...
class DBClient extends mysqli
{
     ...
     *
     * @param string|array $ConnectionParams
     * @param bool   $ThrowExceptions
     *
     * @throws Exception
     */
    public function __construct($ConnectionParams, $ThrowExceptions = true)
    {
        $CS = array();
        if (is_array($ConnectionParams)) {
            $CS['server'] = array_key_exists('host', $ConnectionParams)
                ? $ConnectionParams['host']
                : 'localhost';
            $CS['username'] = array_key_exists('username', $ConnectionParams)
                ? $ConnectionParams['username']
                : '';
            $CS['password'] = array_key_exists('password', $ConnectionParams)
                ? $ConnectionParams['password']
                : '';
            $CS['database'] = array_key_exists('dbname', $ConnectionParams)
                ? $ConnectionParams['dbname']
                : '';
            $CS['port'] = array_key_exists('port', $ConnectionParams)
                ? $ConnectionParams['port']
                : 3306;
            $CS['socket'] = array_key_exists('socket', $ConnectionParams)
                ? $ConnectionParams['socket']
                : null;
        }

        if (is_string($ConnectionParams)) {
            $CS = explode(';', $ConnectionParams);
    ...
    public function query($query)
    {
        if ($this->connect_errno) {
            if ($this->throw_exceptions) {
                throw new Exception(
                    MYSQL_ERSTR_NOT_CONNECTED, MYSQL_ER_NOT_CONNECTED
                );
            }
            return null;
        }
     ...
     */
    public function getValue($query, $type)
    {
     ...
     */
    public function getAffected($query)
    {
        $this->query($query);
    ...
    /**
     * Returns true if the last error code is equal to that given or false
     * otherwise. You may use the constants given
     *
     * @param int $errorCode
     *
     * @return bool
    ...
    public function query_page($query, $recordsPerPage, $Page)
    {
        ...
        if (stripos($query, 'SELECT') === 0) {
            $ret = $this->query(
                substr_replace($query, 'SELECT SQL_CALC_FOUND_ROWS', 0, 6)
                    . ' LIMIT ' . ($recordsPerPage * $Page) . ', '
                    . $recordsPerPage
            );
        }
        return $ret;
    ...
    /**
     * Returns the total number of rows in the last paginated query. DO NOT run
     * unless a paginated query has been ran previously. It will just return 0
     *
     * @return int
     */
    public function query_total()
    {
        return $this->getValue("SELECT FOUND_ROWS()", 'int');
    }
}
...
class MysqliResult
{
    ...
    /**
     * Fetches the next row of data and casts the destination variables to
     * types compatible to those in the database
     *
    ...
    public function fetch_object()
    {
        ...
        foreach ($Row as $Key => $Field) {
            if (in_array(
                $this->FieldsInfo[$k]->type, $GLOBALS['MYSQL2PHP_TYPE_NULL']
            )
            ) {
                $Row->$Key = null;
            } elseif (in_array(
                $this->FieldsInfo[$k]->type, $GLOBALS['MYSQL2PHP_TYPE_INT']
            )
            ) {
                $Row->$Key = intval($Field);
            }
            elseif (in_array(
                $this->FieldsInfo[$k]->type, $GLOBALS['MYSQL2PHP_TYPE_FLOAT']
            )
            ) {
                $Row->$Key = floatval($Field);
            }
            elseif (in_array(
                $this->FieldsInfo[$k]->type, $GLOBALS['MYSQL2PHP_TYPE_STRING']
            )
            ) {
