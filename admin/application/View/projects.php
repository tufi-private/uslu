...
<form action="index.php?show=<?= $this->identifier; ?>&do=meta"
      method="post" class="niceform"
      id="form-<?= $this->identifier; ?>-meta">
    <fieldset>
        <legend>Meta Informationen</legend>
        <dl>
            <dt><label for="page-title">Title:</label></dt>
            <dd><input type="text" name="page-title" id="page-title"
                       size="32" maxlength="128"
                       value="<?= $this->title; ?>"/></dd>
            <dt><label for="page-description">Description:</label></dt>
            <dd><textarea name="page-description" id="page-description"
                          cols="60"
                          rows="5"><?= $this->description; ?></textarea>
            </dd>
        </dl>
    </fieldset>
    <fieldset class="action right-align">
        <input type="submit" name="form-<?= $this->identifier; ?>-submit-meta"
               id="form-<?= $this->identifier; ?>-submit-meta"
               value="Speichern"/>
    </fieldset>
</form>
...
<div id="objects-tab-container">
    ...
    <?php foreach ($this->innerPages as $key => $innerPage) : ?>
        <div id="tabs-<?= $key ?>">
            <h2 id="title-object-<?=$innerPage['id']?>"
                title="Zum Bearbeiten anklicken."
                class="editable"><?= $innerPage['title'] ?></h2>
            ...
            <form method="POST"
                ...
                <div class="right-align">
                    <input type="submit"
                           name="form-<?= $this->identifier; ?>-submit"
                           id="form-<?= $this->identifier; ?>-submit-meta"
                           value="Speichern"/>
                </div>
            </form>
            ...
            <form action="index.php?show=<?= $this->identifier; ?>&do=upload"
                ...
                <fieldset>
                    ...
                    <table>
                        <tr>
                            ...
                            <td>
                                <input type="file" id="input-upload-pdf"
                                       name="pdf-file"/>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <label style="width: 340px;"
                                       for="input-upload-pdf-thumbnail">PDF
                                    Vorschau-Image: </label>
                            </td>
                            <td>
                                <input type="file"
                                       id="input-upload-pdf-thumbnail"
                                       name="pdf-thumbnail-file"/>
                            </td>
                        </tr>
                    </table>
                ...
                <fieldset class="action right-align">
                    <input type="submit"
                           name="form-<?= $this->identifier; ?>-submit-pdf"
                           id="form-<?= $this->identifier; ?>-submit-pdf"
                           value="Speichern"/>
                </fieldset>
            </form>


            <h3 style="padding: 0;">Zugehörige Assets:</h3>

            <div id="asset-list-<?= $innerPage['id'] ?>">
                <?php
                foreach ($innerPage['assets'] as $assetKey => $assetDetails)
                    : ?>
                    <div style="overflow:hidden;font-size: 0.7em;border-bottom: 1px dotted #333333; clear: right; margin-bottom: 1em; padding-bottom: 1em;">
                        ...
                        <?php if (\App\Lib\Image\ImageHandler::isImage(
                            $assetDetails['path']
                        )
                        ): ?>
                            <img src="/<?= Bootstrap::getFrontendBasePath(
                            );?><?= $assetDetails['path']?>"
                                 alt="<?= $assetDetails['path'] ?>"
                                 style="float: right;" class="thumbnail">
                            <?php else: ?>
                            <a href="/<?= Bootstrap::getFrontendBasePath(
                            );?><?= $assetDetails['path']?>"
                               style="float: right;">
                                <?php if (!empty($assetDetails['thumbnail_path'])): ?>
                                <img src="/<?= Bootstrap::getFrontendBasePath(
                                );?><?= $assetDetails['thumbnail_path']?>"
                                     alt="<?= $assetDetails['path'] ?>"
                                     style="float: right;"
                                     class="thumbnail">
                                <?php else: ?>
                                /<?=
                                Bootstrap::getFrontendBasePath()
                                ; ?><?=
                                $assetDetails['path'] ?>
                                <?php endif; ?>
                            </a>
                        ...
                        foreach ($assetDetails as $property => $value) : ?>
                            ...
                            <p style="margin:0;"><strong><?= ucfirst(
                                str_replace('_', ' ', $property)
                            ) ?>:</strong> <?= $value ?></p>
                            <?php endforeach; ?>
                        <form action="index.php?show=<?= $this->identifier ?>>&do=deleteAssets"
                              method="post">
                            <input type="hidden" name="assetId[]"
                                   value="<?= $assetDetails['id'] ?>"/>
                            <input type="submit" name="delete_asset"
                                   value="Löschen" style="float: right;"/>
                        </form>
                    </div>
                    <?php endforeach; ?>
            </div>
...
<form action="/<?= Bootstrap::getBasePath() ?>index.php" method="post"
    ...
    <fieldset>
        ...
        <dl>
            <dt><label for="page-uploadBgImage">Hintergrundbild: </label>
            </dt>
            <dd>
                <input type="file" name="page-uploadBgImage"
                       id="page-uploadBgImage"/>
            </dd>
        </dl>
        <p>
            <?php if (trim($this->backgroundImage) != ''): ?>
            <img src="/<?= Bootstrap::getBasePath(
